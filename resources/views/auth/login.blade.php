...
@section('content')
    <x-forms.auth-form title="Вход в аккаунт" action="{{ route('login.handle') }}" method="POST">
        @csrf
        ...
        <x-slot:links>
            <div class="space-y-3 mt-5">
                <div class="text-xxs md:text-xs">
                    <a href="{{ route('forgot') }}" class="text-gray hover:text-gray/50">
                        Забыли пароль?
                    </a>
                </div>
                <div class="text-xxs md:text-xs">
                    <a href="{{ route('register') }}" class="text-gray hover:text-gray/50">
                        Регистрация
                    </a>
                </div>
            </div>
        ...
        <x-slot:agreements>
            <ul class="flex flex-col md:flex-row justify-between gap-3 md:gap-4 mt-12 md:mt-14">
                <li>
                    <a href="#" class="inline-block text-gray hover:text-gray/50 text-xxs md:text-xs font-medium"
