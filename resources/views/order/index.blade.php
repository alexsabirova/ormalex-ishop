    ...
    <main class="py-16 lg:py-20">
        <div class="container">
            ...
            <ul class="breadcrumbs flex flex-wrap gap-y-1 gap-x-4 mb-6">
                ...
                <li><a href="{{ route('cart') }}" class="text-gray hover:text-pink text-xs">Корзина покупок</a></li>
                <li><span class="text-teal-700 text-xs">Оформление заказа</span></li>
            </ul>

            <section>
                <!-- Section heading -->
                <h1 class="mb-8 text-lg lg:text-[36px] font-medium text-dark">Оформление заказа</h1>
                ...
                <form action="{{ route('order.handle') }}" method="POST"
                    ...
                    <!-- Contact information -->
                    <div class="p-6 2xl:p-8 rounded-[20px] bg-gradient-to-r from-neutral-50 to-neutral-100 shadow-xl">
                        <h3 class="mb-6 text-md 2xl:text-lg font-medium">
                            ...
                            @guest
                                <div x-data="{ createAccount: false }">
                                    ...
                                    <div>
                                        ...
                                        >
                                        <label
                                            for="checkout-create-account" class=" text-gray pl-4"
                    ...
                    <div class="space-y-6 2xl:space-y-8">
                        <div class="p-6 2xl:p-8 rounded-[20px] bg-gradient-to-r from-neutral-50 to-neutral-100 shadow-xl">
                            <h3 class="mb-6 text-md 2xl:text-lg font-medium">Метод оплаты</h3>
                            <div class="space-y-5">
                                @foreach($payments as $payment)
                                    <div>
                                        ...
                                        <label for="payment-method-{{ $payment->id }}" class="form-radio-label pl-4">
                                            {{ $payment->title }}
                                        </label>
                                    </div>
                                @endforeach
                            </div>
                            <h3 class="my-6 text-md 2xl:text-lg font-medium">
                                Способ доставки
                            </h3>
                            <div class="space-y-5">
                    ...
                    <!-- Checkout -->
                    <div class="p-6 2xl:p-8 rounded-[20px] bg-gradient-to-r from-neutral-50 to-neutral-100 shadow-xl">
                        <h3 class="mb-6 text-md 2xl:text-lg font-medium">Заказ</h3>
                        <table class="w-full border-spacing-y-3 text-body text-xxs text-left"
                               style="border-collapse: separate">
                            <thead class="text-[12px] text-gray uppercase">
                            <tr>
                            ...
                            @foreach($items as $item)
                                <tr>
                                    <td scope="row" class="pb-3 border-b border-body/10">
                                        <h4 class="font-bold">
                                            <a href="{{ route('product', $item->product) }}"
                                               class="inline-block text-gray hover:text-pink break-words pr-3">
                                                {{ $item->product->title }}
                                            </a>
                                        </h4>
                                    ...
                                    <td class="px-2 pb-3 border-b border-gray/20 text-gray whitespace-nowrap">
                                        {{ $item->quantity }} шт.
                                    </td>
                                    <td class="px-2 pb-3 border-b border-gray/20 text-gray whitespace-nowrap">
                                        {{ $item->amount }}
                        ...
                        <div class="text-xs font-semibold text-teal-700 text-right">
                            Всего: {{ cart()->amount() }}
                        </div>
                        ...
                        <div class="mt-8 space-y-8">
                            ...
                            <input type="hidden" name="amount" value="{{ cart()->amount()->raw() }}">
                            <!-- Process to checkout -->
                            <button type="submit" class="w-full btn btn-teal">Оформить заказ</button>
                        </div>
                    </div>
                </form>
            </section>
