<header class="header fixed inset-x-0 z-50 px-4 py-4 xl:pt-6 bg-white shadow-lg">
    <div class="header-inner flex items-center justify-between lg:justify-start pb-1 px-20">
        <div class="header-logo shrink-0">
            <a href="{{ route('home') }}" rel="home">
                <img src="{{ Vite::image('logo-origin.svg') }}"
                     class="w-[150px] xs:w-[148px] md:w-[300px] h-[30px] xs:h-[36px] md:h-[50px]" alt="Ormalex">
            </a>
        </div><!-- /.header-logo -->
        <div class="header-menu grow hidden lg:flex items-center ml-8 mr-8 gap-8">
            <form action="{{ route('catalog') }}" class="hidden lg:flex gap-3">
                <input
                    ...
                    type="search"
                    class="w-full h-12 px-4 rounded-lg border border-body/50 focus:border-teal-200 focus:shadow-[0_0_0_3px_#00babe] bg-white/5 text-neutral-600 text-xs shadow-transparent outline-0 transition"
                    placeholder="Поиск..."
                    required
                >
                <button type="submit" class="shrink-0 w-12 !h-12 !px-0 btn btn-teal">
        ...
        <div class="header-actions flex items-center gap-3 md:gap-5">
                ...
                <div x-data="{dropdownProfile: false}" class="profile relative">
                    <button @click="dropdownProfile = ! dropdownProfile"
                        ...
                        <img src="{{ auth()->user()->avatar }}" class="shrink-0 w-7 md:w-9 h-7 md:h-9 rounded-full"
                             alt="{{ auth()->user()->name }}">
                        <span class="hidden md:block ml-2 font-medium text-teal-800">{{ auth()->user()->name }}</span>
                    ...
                    <div
                        ...
                        class="absolute z-50 top-0 -right-20 xs:-right-8 sm:right-0 w-[280px] sm:w-[300px] mt-14 p-4 rounded-lg shadow-xl bg-neutral-100 shadow-lg"
                    >
                        <h5 class="text-teal-900 text-xs">Мой профиль</h5>
                            ...
                            <ul class="space-y-2">
                                <li>
                                    <a href="{{ route('wishlist') }}"
                                       class="text-gray hover:text-teal-700 text-xs font-medium">
                                        Избранное
                                    </a>
                                </li>
                                <li>
                                    <a href="{{ route('orders.list') }}" class="text-gray hover:text-teal-700 text-xs font-medium">
                                        История заказов
                                    </a>
                                </li>
                                <li>
                        ...
                        <div class="mt-6">
                            <form action="{{ route('logout') }}" method="POST">
                                ...
                                <button type="submit" class="inline-flex items-center text-body hover:text-pink">
                                    ...
                                    <span class="ml-2 font-medium text-gray hover:text-pink">Выйти</span>
                                </button>
                            </form>
                        </div>
                    </div>
            ...
            @elseguest
                <a href="{{ route('login') }}" class="profile hidden xs:flex items-center">
                    ...
                    <span class="profile-text relative ml-2 text-dark text-xxs md:text-xs font-bold">Войти</span>
                </a>
            @endauth

            <a href="{{ route('wishlist') }}" class="max-2xl:hidden flex items-center gap-3 text-pink hover:text-teal-600">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-6 md:w-7 w-6 md:h-7" viewBox="0 0 24 24"
                     fill="currentColor">
                    <path
            ...
            <a href="{{ route('cart') }}" class="flex items-center gap-3 text-pink hover:text-teal-600">
                ...
                <div class="hidden sm:flex flex-col gap-1">
                    <span class="text-neutral-600 text-xxs leading-none">{{ cart()->count() }} шт</span>
                    <span class="text-teal-800 text-xxs 2xl:text-xs !leading-none">{{ cart()->amount() }}</span>
                </div>
            </a>
            <button id="burgerMenu" class="flex 2xl:hidden text-teal-800 hover:text-pink transition">
